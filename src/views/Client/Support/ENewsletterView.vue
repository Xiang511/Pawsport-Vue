    ...
    <div class="mx-auto w-11/12 lg:w-[95%] xl:w-10/12">
        ...
        <div class="flex gap-8 lg:gap-12">
          <button
            ...
            :class="[
              ...
              activeCategory === cat
                ? 'text-[#7a6856]' 
                : 'text-[#968677] hover:text-[#7a6856]'
            ]">
            {{ cat }}
            <span 
              v-if="activeCategory === cat" 
              class="absolute bottom-0 left-0 h-1 w-full rounded-full bg-[#7a6856]"
            ></span>
          </button>
        </div>
      </div>
      ...
      <div v-else class="grid grid-cols-1 gap-8 md:grid-cols-2 xl:grid-cols-3">
        ...
        <article 
          ...
          <div class="relative flex h-52 w-full items-center justify-center overflow-hidden bg-[#e8dccb]/40">
            <template v-if="enews.image">
              <img :src="enews.image" class="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" alt="電子報封面" />
            </template>
            <template v-else>
              <ImageIcon class="h-12 w-12 text-[#d8ccbb]" />
            </template>
            
            <span class="absolute left-4 top-4 rounded-full bg-[#fcf7c6] px-4 py-1.5 text-sm font-bold text-[#755e44] shadow-sm">
              {{ enews.category }}
            </span>
          </div>

          <div class="flex flex-grow flex-col p-6 lg:p-8">
            <h2 class="mb-3 line-clamp-2 text-xl font-bold text-[#7a6856] transition-colors group-hover:text-[#5c4e40]">
              {{ enews.title }}
            </h2>
            ...
            <div class="mt-auto flex items-center justify-between border-t-2 border-dashed border-[#e8dccb] pt-4">
              <div class="flex items-center gap-2 text-sm text-[#968677]">
                <Calendar class="h-4 w-4" />
                <span>{{ formatDate(enews.publishDate) }}</span>
              </div>
              
              <div class="flex items-center gap-1 font-bold text-[#755e44] transition-transform group-hover:translate-x-1">
                閱讀更多
      ...
      <div v-if="!isLoading && filteredEnews.length === 0" class="py-12 text-center text-lg text-[#968677]">
        這個分類目前還沒有電子報喔！
      </div>
    </div>
  </div>
