  ...
  <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
    ...
    <div class="overflow-x-auto">
      <table class="w-full table-auto text-left">
        ...
        <tbody v-if="!loading">
          <tr v-for="item in paginatedRecords" :key="item.adoptionId" class="border-b dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-white/[0.02]">
            ...
            <td class="px-4 py-4 text-center text-sm">
              <button @click="goToEdit(item.adoptionId)" class="mr-3 font-medium text-primary hover:underline">編輯</button>
              <button @click="deleteRecord(item.adoptionId)" class="font-medium text-red-500 hover:underline">刪除</button>
            </td>
          </tr>
        </tbody>
      </table>
      ...
      <div v-else-if="allRecords.length === 0" class="py-20 text-center text-gray-500">尚無領養紀錄</div>
    </div>

    <div class="mt-6 flex items-center justify-between">
      <p class="text-sm text-gray-600 dark:text-gray-400">
        顯示第 {{ (currentPage - 1) * pageSize + 1 }} 到 {{ Math.min(currentPage * pageSize, allRecords.length) }} 筆，共 {{ allRecords.length }} 筆
      </p>
      <div class="flex gap-2">
        <button 
