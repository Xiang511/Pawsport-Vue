...
// 使用 Algolia 搜尋菜單項目（使用 menu 索引）
const { query, hits, isLoading, error, clearResults } = useAlgoliaSearch('menu')
  ...
  <div class="relative hidden lg:block">
    <div class="relative">
      ...
      <button
        v-if="query"
        @click="handleClear"
        class="absolute top-1/2 right-2.5 flex h-6 w-6 -translate-y-1/2 items-center justify-center text-xl leading-none text-gray-400 hover:text-gray-600 dark:hover:text-gray-200">
        &times;
      </button>
      <button
        v-else
        class="pointer-events-none absolute top-1/2 right-2.5 inline-flex -translate-y-1/2 items-center gap-0.5 rounded-lg border border-gray-200 bg-gray-50 px-[7px] py-[4.5px] text-xs -tracking-[0.2px] text-gray-500 dark:border-gray-800 dark:bg-white/[0.03] dark:text-gray-400">
        <span>⌘</span>
        <span>K</span>
      </button>
    </div>
          ...
          <button
            ...
            <div class="min-w-0 flex-1">
              <div class="mb-1 flex items-center gap-2">
                ...
                <span
                  v-if="hit.isPro"
                  class="inline-flex items-center rounded bg-purple-100 px-1.5 py-0.5 text-xs font-medium text-purple-700 dark:bg-purple-900 dark:text-purple-300">
                  PRO
                </span>
                <span
                  v-if="hit.isNew"
                  class="inline-flex items-center rounded bg-green-100 px-1.5 py-0.5 text-xs font-medium text-green-700 dark:bg-green-900 dark:text-green-300">
                  NEW
                </span>
              </div>
